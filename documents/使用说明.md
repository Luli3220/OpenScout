...
- `src/run_pipeline.py`：数据初始化/新用户自动挖掘流水线（按用户名运行）。
...
```bash
# 为单个用户跑一遍数据采集与计算（生成 data/raw_users/{username}/ 下所需文件）
python src/run_pipeline.py --username <github_id>
```
...
- 增加新的采集步骤：在 `src/` 添加脚本并在 `src/run_pipeline.py` 中注册执行顺序。\
