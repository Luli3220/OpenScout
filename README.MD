...
- **维护力 (Maintainership)**
  ```text
  S_main = 0.7 * Merged Others' PRs + 0.3 * Review Comments
  ```
...
在开发者详情页的「代表仓库」列表中，支持对单个仓库进行基于大模型的自动介绍与总结：系统会根据仓库 URL 自动拉取 `README.md`、Repository Description 与 Topics，并调用 DeepSeek 生成一段简洁的仓库说明，支持流式输出展示。
